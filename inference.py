...
SCENARIOS = [
    ...
    },
    {
        ...
        "label": 0,
        "context": "Classic urgency scam with a weak sender signal.",
...
def observation_to_list(observation: np.ndarray | list[float]) -> list[float]:
...
class EmailTriageEnv(gym.Env):
    ...
    def __init__(self, task_level: str = DEFAULT_TASK_LEVEL, max_steps: int = MAX_STEPS, seed: int | None = None):
        ...
        self._current_scenario = SCENARIOS_BY_LABEL[1][0]
        self._needs_reset = True

    def _sample_label(self) -> int:
        return int(self._rng.choice(len(ACTION_NAMES), p=LABEL_WEIGHTS))

    def _sample_scenario(self, label: int) -> dict[str, Any]:
        return SCENARIOS_BY_LABEL[label][int(self._rng.integers(0, len(SCENARIOS_BY_LABEL[label])))]
    ...
    def _build_observation(self, scenario: dict[str, Any], label: int) -> np.ndarray:
        ...
        if float(self._rng.random()) < config["blend_prob"]:
            distractor_label = int(self._rng.choice(CONFUSER_LABELS[label]))
            distractor = self._sample_scenario(distractor_label)
            alpha = float(self._rng.uniform(*config["blend_alpha"]))
        ...
        noise = self._rng.normal(0.0, config["noise_std"], size=self.observation_space.shape).astype(np.float32)
        observation = np.clip(observation + noise, 0.0, 1.0)

        if float(self._rng.random()) < config["flip_prob"]:
            indices = self._rng.choice(np.arange(len(FEATURE_NAMES)), size=2, replace=False)
            observation[indices] = np.clip(
                observation[indices] + self._rng.normal(0.0, config["noise_std"] * 1.5, size=2),
                0.0,
                1.0,
            )
    ...
    def reset(self, *, seed: int | None = None, options: dict[str, Any] | None = None) -> tuple[np.ndarray, dict[str, Any]]:
        ...
        if isinstance(options, dict):
            task_level = options.get("task_level")
            if isinstance(task_level, str) and task_level in TASK_LEVELS:
                self.task_level = task_level
...
def llm_agent_sim(observation: np.ndarray, rng: np.random.Generator) -> int:
    ...
    scores = np.array(
        [
            spam * 0.90 + (1.0 - sender) * 0.55 + keyword * 0.12,
            sender * 0.68 + (1.0 - spam) * 0.34 + attachment * 0.18 + keyword * 0.08,
            social * 0.95 + (1.0 - urgency) * 0.10 + (1.0 - promo) * 0.05,
            promo * 0.92 + keyword * 0.16 + (1.0 - reply) * 0.08,
            urgency * 0.88 + keyword * 0.25 + sender * 0.20 + (1.0 - spam) * 0.10 + time_of_day * 0.02,
            reply * 0.92 + thread_length * 0.25 + urgency * 0.18 + sender * 0.10,
        ],
        dtype=np.float32,
    )
    scores += rng.normal(0.0, 0.03, size=len(ACTION_NAMES))
...
def _service_is_ready(host: str, port: int, timeout: float = 1.0) -> bool:
    ...
    for path, expectation in endpoints:
        ...
        if statuses and payload.get("status") in statuses:
            return True
...
app = FastAPI(
    ...
    version="1.0.0",
    description="OpenEnv-compatible Gymnasium environment for Indian email triage.",
    lifespan=lifespan,
)
...
async def reset_endpoint(body: ResetRequest | None = Body(default=None)) -> ResetResponse:
    ...
    try:
        with _env_lock:
            ...
            observation, _ = _env.reset(seed=payload.seed, options=options)

        return ResetResponse(observation=observation_to_list(observation), info={})
...
@app.post("/step/agent", response_model=AgentStepResponse)
...
if __name__ == "__main__":
    ...
    cli_mode_env = os.getenv("CLI_MODE", "true").lower()
    cli_mode = cli_mode_env not in ("false", "0", "no")

    if server_mode or not cli_mode:
        raise SystemExit(run_server())
    raise SystemExit(run_cli_inference())
